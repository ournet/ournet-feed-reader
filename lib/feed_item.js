...
exports.process = function(item, result) {
	var culture = {
	...
	return htmlExplorer.explore(item.url, {
		...
		.then(function(page) {
			return internal.processPage(page, item, result, culture);
		});
};

internal.processPage = function(page, item, result, culture) {
	if (!page) {
		return null;
	}
...
internal.normalizeUrl = function(link) {
	...
	return 'http://' + link.host + link.path;
};
